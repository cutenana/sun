...
<div class="container">
	<div class="row">
	    ...
	    <?php $dosql->Execute("SELECT * FROM `#@__infolist` WHERE classid=20 AND delstate='' AND checkinfo=true ORDER BY orderid ASC LIMIT 0,1");
				while($row = $dosql->GetArray())
				{
					//获取链接地址
					if($row['linkurl']=='' and $cfg_isreurl!='Y')
						...
						$gourl = $row['linkurl'];
				?>
             <div class="a-content-text">   <?php echo $row['content']; ?>
           </div>
           <?php
				}
				?>
